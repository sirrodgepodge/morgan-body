...
var clfdate = require("./util/dateFormatter")


function standardRequestLineCheck(line) {
  console.log(new Date().toUTCString());
  expect(line).to.equal(
    `\u001b[96mRequest: \u001b[93mGET \u001b[97m/ \u001b[90mat \u001b[37m${new Date().toUTCString()}, \u001b[90mIP: \u001b[37m::ffff:127.0.0.1\u001b[0m, \u001b[90mUser Agent: node-superagent/3.8.3\u001b[0m\n`
  );
}
...
function standardResponseLineCheck(line) {
  expect(forceResponseTimeToZero(line)).to.equal(
    `\u001b[96mResponse: \u001b[32m200 \u001b[37m0.000 ms \n`
  );
}
...
`)
    ...
    simulateRequestPromise({ maxBodyLength: 100 }, "post", {
      key: "value",
      long:
        "aodshglkdgdlbgkz,bcmnxbvmcbgnsdlajkghdsalkjghxzlkgndajlkgㅓㅏ홍허ㅏ아ㅓ힝히ㅏㅓㅗㅇ마힝히머ㅓㅏㅣ론aksdjlgsjgklhdsgjksdahgakljsdgdskjlghdsajkladklsjghsdjkghslkjghdskjghdszgjkhda",
    });
  ...
  it('should respect "logReqDateTime" property', function () {
    const consoleTestPromise = consoleTest((line) =>
      expect(line).to.equal(
  ...
  it('should respect "logResponseBody" property', function () {
    ...
    simulateRequestPromise({ logResponseBody: false }, "get", null, {
      key: "val",
    });
  ...
  it('should respect "logRequestId" property, example without req ID being given', function () {
    const consoleTestPromise = consoleTest(
      ...
      (line) =>
        expect(line).to.equal(
          `\u001b[95m[-] Response Body:\u001b[0m\n\u001b[97m{\u001b[0m\n\u001b[97m\t"key": "resVal"\u001b[0m\n\u001b[97m}\u001b[0m\n`
        ),
      (line) =>
        expect(forceResponseTimeToZero(line)).to.equal(
          `\u001b[96m[-] Response: \u001b[32m200 \u001b[37m0.000 ms \n`
        )
    );
  ...
  it('should respect "logRequestId" property, example with req ID being given', function () {
    ...
    const consoleTestPromise = consoleTest(
      ...
      (line) =>
        expect(line).to.equal(
          `\u001b[95m[${reqId}] Request Body:\u001b[0m\n\u001b[97m{\u001b[0m\n\u001b[97m\t"key": "reqVal"\u001b[0m\n\u001b[97m}\u001b[0m\n`
        ),
      (line) =>
        expect(line).to.equal(
          `\u001b[95m[${reqId}] Response Body:\u001b[0m\n\u001b[97m{\u001b[0m\n\u001b[97m\t"key": "resVal"\u001b[0m\n\u001b[97m}\u001b[0m\n`
        ),
      (line) =>
        expect(forceResponseTimeToZero(line)).to.equal(
          `\u001b[96m[${reqId}] Response: \u001b[32m200 \u001b[37m0.000 ms \n`
        )
    );
  ...
  it('should warn if "timezone" provided with "dateTimeFormat" of "utc"', function () {
    ...
    simulateRequestPromise(
      { dateTimeFormat: "utc", timezone: "Africa/Blantyre" },
      "get"
    );
  ...
  it('"timezone" property should update GMT adjustment val for "iso" "dateTimeFormat"', function () {
    ...
    simulateRequestPromise(
      { dateTimeFormat: "iso", timezone: "Africa/Blantyre" },
      "get"
    );
  ...
  it('"timezone" property should update GMT adjustment val for "clf" "dateTimeFormat"', function () {
    ...
    simulateRequestPromise(
      { dateTimeFormat: "clf", timezone: "Africa/Blantyre" },
      "get"
    );
  ...
  it('"stream" property should enable writing somewhere other than stdout', function () {
    ...
    return simulateRequestPromise(
      { stream: { write: (text) => arr.push(text) } },
      "get"
    ).then(() => {
...
`;
    ...
    it('"inverted" theme should invert colors', function () {
      ...
      const consoleTestPromise = consoleTest(
        ...
        (line) =>
          expect(line).to.equal(`\u001b[92mRequest Body:\u001b[0m\n` + bodyStr),
        (line) =>
          expect(line).to.equal(
            `\u001b[92mResponse Body:\u001b[0m\n` + bodyStr
          ),
        (line) =>
          expect(forceResponseTimeToZero(line)).to.equal(
            `\u001b[91mResponse: \u001b[35m200 \u001b[30m0.000 ms \n`
          )
      );
    ...
    it('"dimmed" theme should have no intense colors', function () {
      ...
      const consoleTestPromise = consoleTest(
        ...
        (line) =>
          expect(line).to.equal(`\u001b[35mRequest Body:\u001b[0m\n` + bodyStr),
        (line) =>
          expect(line).to.equal(
            `\u001b[35mResponse Body:\u001b[0m\n` + bodyStr
          ),
        (line) =>
          expect(forceResponseTimeToZero(line)).to.equal(
            `\u001b[36mResponse: \u001b[32m200 \u001b[37m0.000 ms \n`
          )
      );
    ...
    it('"darkened" theme should push all colors down by one unless black', function () {
      ...
      const consoleTestPromise = consoleTest(
        ...
        (line) =>
          expect(line).to.equal(`\u001b[94mRequest Body:\u001b[0m\n` + bodyStr),
        (line) =>
          expect(line).to.equal(
            `\u001b[94mResponse Body:\u001b[0m\n` + bodyStr
          ),
        (line) =>
          expect(forceResponseTimeToZero(line)).to.equal(
            `\u001b[95mResponse: \u001b[31m200 \u001b[36m0.000 ms \n`
          )
      );
    ...
    it('"lightened" theme should push all colors up by one unless black', function () {
      ...
      const consoleTestPromise = consoleTest(
        ...
        (line) =>
          expect(line).to.equal(`\u001b[96mRequest Body:\u001b[0m\n` + bodyStr),
        (line) =>
          expect(line).to.equal(
            `\u001b[96mResponse Body:\u001b[0m\n` + bodyStr
          ),
        (line) =>
          expect(forceResponseTimeToZero(line)).to.equal(
            `\u001b[97mResponse: \u001b[33m200 \u001b[37m0.000 ms \n`
          )
      );
    ...
    it('"dracula" theme should be dracula-ish', function () {
      ...
      const consoleTestPromise = consoleTest(
        ...
        (line) =>
          expect(line).to.equal(`\u001b[97mRequest Body:\u001b[0m\n` + bodyStr),
        (line) =>
          expect(line).to.equal(
            `\u001b[97mResponse Body:\u001b[0m\n` + bodyStr
          ),
        (line) =>
          expect(forceResponseTimeToZero(line)).to.equal(
            `\u001b[97mResponse: \u001b[31m200 \u001b[97m0.000 ms \n`
          )
      );
    ...
    it('"usa" theme should be usa colors', function () {
      ...
      const consoleTestPromise = consoleTest(
        ...
        (line) =>
          expect(forceResponseTimeToZero(line)).to.equal(
            `\u001b[31mResponse: \u001b[31m200 \u001b[97m0.000 ms \n`
          )
      );
